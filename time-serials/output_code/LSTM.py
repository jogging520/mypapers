...
if __name__ == "__main__":
    ...
    # 9. design LSTM network
    model = Sequential()
    model.add(LSTM(64, input_shape=(time_step,1)))
    model.add(Dense(time_step))
    model.compile(loss='mae', optimizer='adam')
   # 10 fit network
    history = model.fit(trainX, train_Y, epochs=10, batch_size=14, verbose=2)

    # 11 make prediction
    testPredict = model.predict(testX)
    print('testPredict', testPredict.shape)
    # 12 invert predictions
    inv_yhat  = scaler.inverse_transform(testPredict)
